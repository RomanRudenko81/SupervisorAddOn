(function () {
  const TAG = 'supervisor-header-widget-v21';
  const GLOBAL = '__wxcc_header_status_v21__';
  ...
  class SupervisorHeaderWidgetV19 extends HTMLElement {
    ...
    _updateBrowserIndicators(agentName, state, ok) {
      try {
        ...
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><circle cx="32" cy="32" r="29" fill="${color}" stroke="${border}" stroke-width="4"/><circle cx="22" cy="20" r="7" fill="rgba(255,255,255,0.88)"/></svg>`;
        const href = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(svg);

        const docs = [];
        if (document && document.head) docs.push(document);
        try {
          if (window.top && window.top.document && window.top.document.head && !docs.includes(window.top.document)) {
            docs.push(window.top.document);
          }
        } catch (e) {}
        try {
          if (window.parent && window.parent.document && window.parent.document.head && !docs.includes(window.parent.document)) {
        ...
        for (const doc of docs) {
          try {
            doc.title = `${emoji} ${cleanAgent} | ${cleanState}`;

            // Force-create favicon links. Some WXCC pages have no favicon link at all.
            let icon = doc.getElementById('wxcc-header-status-favicon');
            if (!icon) {
              icon = doc.createElement('link');
              icon.id = 'wxcc-header-status-favicon';
              icon.rel = 'icon';
            ...
            let shortcut = doc.getElementById('wxcc-header-status-shortcut-favicon');
            if (!shortcut) {
              shortcut = doc.createElement('link');
              shortcut.id = 'wxcc-header-status-shortcut-favicon';
            ...
            // Also update any existing favicon links Cisco may create later.
            const existing = Array.from(doc.querySelectorAll('link[rel*="icon" i]'));
  ...
  if (!customElements.get(TAG)) customElements.define(TAG, SupervisorHeaderWidgetV21);
})();
