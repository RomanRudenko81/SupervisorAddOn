(function () {
  const TAG = 'supervisor-header-widget-v22';
  const GLOBAL = '__wxcc_header_status_v22__';
  ...
  class SupervisorHeaderWidgetV19 extends HTMLElement {
    ...
    _updateBrowserIndicators(agentName, state, ok) {
      try {
        const color = ok ? '#198754' : '#B15A52';
        const border = ok ? '#0E6A3E' : '#8F403A';
        const emoji = ok ? '🟢' : '🔴';
        const cleanAgent = normalizeText(agentName || 'WXCC');
        const cleanState = normalizeText(state || 'Status');

        // Browser tab title, visible even when the widget/page is in the background.
        document.title = `${emoji} ${cleanAgent} | ${cleanState}`;

        // Dynamic tab favicon: small green/red status dot.
        const svg = `
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
            <circle cx="32" cy="32" r="29" fill="${color}" stroke="${border}" stroke-width="4"/>
            <circle cx="22" cy="20" r="7" fill="rgba(255,255,255,0.85)"/>
          </svg>`;
        const href = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(svg);

        let icon = document.querySelector('link[data-wxcc-header-status-favicon="true"]');
        if (!icon) {
          icon = document.createElement('link');
          icon.setAttribute('rel', 'icon');
          icon.setAttribute('type', 'image/svg+xml');
          icon.setAttribute('data-wxcc-header-status-favicon', 'true');
    ...
    _safeUpdateFavicon(agentName, state, ok) {
      // Safe/non-blocking: never affects widget rendering if browser/WXCC blocks favicon changes.
      try {
        if (this._faviconTimer) window.clearTimeout(this._faviconTimer);
        this._faviconTimer = window.setTimeout(() => {
          try {
            const doc = document;
            if (!doc || !doc.head) return;
            const cleanAgent = normalizeText(agentName || 'Supervisor');
            const cleanState = normalizeText(state || 'Status');
            const emoji = ok ? '🟢' : '🔴';

            // Title change is harmless; WXCC may overwrite it again.
            try { doc.title = `${emoji} ${cleanAgent} | ${cleanState}`; } catch (e) {}

            // Use PNG data URL from canvas instead of SVG data URL to avoid CSP/SVG favicon issues.
            let href = '';
            try {
              ...
              ctx.fillStyle = ok ? '#198754' : '#B15A52';
              ctx.fill();
              ctx.lineWidth = 4;
              ctx.strokeStyle = ok ? '#0E6A3E' : '#8F403A';
              ctx.stroke();
              ctx.beginPath();
              ctx.arc(23, 21, 7, 0, Math.PI * 2);
            ...
            let link = doc.getElementById('wxcc-supervisor-status-favicon-v22');
            if (!link) {
              link = doc.createElement('link');
              link.id = 'wxcc-supervisor-status-favicon-v22';
              link.rel = 'icon';
              link.type = 'image/png';
    ...
    render() {
      ...
      this._safeUpdateFavicon(agentName, state, ok);
      this._updateBrowserIndicators(agentName, state, ok);
  ...
  if (!customElements.get(TAG)) customElements.define(TAG, SupervisorHeaderWidgetV22);
})();
