(function () {
  const TAG = 'supervisor-header-widget-v6';

  class SupervisorHeaderWidgetV6 extends HTMLElement {
    ...
    _collectTextDeep(root, out, depth = 0) {
      ...
      for (const el of nodes) {
        try {
          ...
          if (txt) out.push(txt);
          if (el.shadowRoot) this._collectTextDeep(el.shadowRoot, out, depth + 1);
        } catch (e) {}
      }
    }
    ...
    render() {
      ...
      const bg = ok ? '#18864B' : '#B3261E';
      const border = ok ? '#0E5F35' : '#7A1612';
      const glow = ok ? 'rgba(24,134,75,.35)' : 'rgba(179,38,30,.35)';
